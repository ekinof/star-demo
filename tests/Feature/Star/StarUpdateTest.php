...
test('users can update a star with valid data', function () {
    $user = User::factory()->create();
    $star = Star::factory()->create();

    $response = $this->actingAs($user)->put('/api/stars/'.$star->id, [
        'firstName' => 'Emma',
        'lastName' => 'Stone',
...
test('guest cannot update a star with valid data', function () {
    $star = Star::factory()->create();

    $response = $this->put('/api/stars/'.$star->id, [
        'firstName' => 'Emma',
        'lastName' => 'Stone',
        'imageUrl' => 'https://upload.wikimedia.org/wikipedia/commons/9/9a/Emma_Stone_at_the_39th_Mill_Valley_Film_Festival_%28cropped%29.jpg',
        'description' => 'Emma Stone est une actrice et productrice américaine, née le 6 novembre 1988 à Scottsdale',
